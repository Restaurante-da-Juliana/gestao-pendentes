    ...
    reader.onload = (e) => {
      try {
        ...
        for (let i = 1; i < jsonData.length; i++) {
          ...
          if (row && row.length >= 6) {
            pedidos.push({
              valor_devido: parseFloat(String(row[0])) || 0,
              descricao: String(row[1] || ''),
              data: formatExcelDate(row[2]),
              telefone: formatPhone(String(row[3] || '')),
              nome_cliente: String(row[4] || ''),
              id: String(row[5]) || String(i),
            });
          }
        }
...
export const formatPhoneDisplay = (phone: string): string => {
  ...
  if (numbers.length === 13) {
    // 55 + DDD + 9 digits
    return `(${numbers.slice(2, 4)}) ${numbers.slice(4, 5)} ${numbers.slice(5, 9)}-${numbers.slice(9)}`;
  }
